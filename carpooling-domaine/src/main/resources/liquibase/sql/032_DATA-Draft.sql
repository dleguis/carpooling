...
/* -- CREATE TABLES */
CREATE TABLE USERS (LOGIN VARCHAR(30) NOT NULL, EMAIL VARCHAR(80), PWD VARCHAR(20) NOT NULL, PERSONNAL_ADR_ID INTEGER, PRIMARY KEY (LOGIN));
CREATE TABLE MAILING_ADDRESSES (ID INTEGER AUTO_INCREMENT NOT NULL, LINE3 VARCHAR(80), ZIP VARCHAR(10), LINE1 VARCHAR(80) NOT NULL, LINE2 VARCHAR(80), CITY VARCHAR(45), PRIMARY KEY (ID));

CREATE TABLE AUTHORS (ID INTEGER AUTO_INCREMENT NOT NULL, LAST_NAME VARCHAR(35) NOT NULL, BIRTH_DATE DATE, FIRST_NAME VARCHAR(35) NOT NULL, PRIMARY KEY (ID));
...
CREATE TABLE ORDERS (ID INTEGER AUTO_INCREMENT NOT NULL, ORDER_DATE TIMESTAMP, USER_ID VARCHAR(30), SHIPPING_ADR_ID INTEGER, PRIMARY KEY (ID));
...
/* -- ADD FK CONSTRAINTS */
ALTER TABLE USERS ADD CONSTRAINT SERSPERSONNALADRID FOREIGN KEY (PERSONNAL_ADR_ID) REFERENCES MAILING_ADDRESSES (ID);

ALTER TABLE BOOKS ADD CONSTRAINT FK_BOOKS_AUTHOR_ID FOREIGN KEY (AUTHOR_ID) REFERENCES AUTHORS (ID);

ALTER TABLE ORDERS ADD CONSTRAINT FK_ORDERS_USER_ID FOREIGN KEY (USER_ID) REFERENCES USERS (LOGIN);
ALTER TABLE ORDERS ADD CONSTRAINT RDERSSHIPPINGADRID FOREIGN KEY (SHIPPING_ADR_ID) REFERENCES MAILING_ADDRESSES (ID);
